import { getProfileById, updateProfile } from '@/lib/profile/data';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const userProfile = await getProfileById(id.toString());
...
export async function PATCH(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const updatedProfile = await updateProfile(id.toString(), data);
